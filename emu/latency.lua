...
-- GENERATED, not transcribed. This file arrived from a sibling with these
-- addresses written in by hand and a comment saying to keep them in step with
-- the assembly. They were not in step, and the failure was silent in both
-- directions: the ROM completed 588 transactions and the harness, tapping
-- three cells that no longer meant anything, reported nought rounds.
-- tools/mksyms.py reads them out of the assembler's own listing.
local SYM = dofile(os.getenv("DMSYMS") or "build/probesyms.lua")
local VOST, VOERR, VOFCNT = SYM.PSTEP, SYM.DMERR, SYM.DMFCNT
local PROUND = SYM.PROUND
