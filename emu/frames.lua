...
-- REPORT ON A FRAME COUNT, NOT ON MACHINE STOP.
--
-- This file arrived from a sibling ending in add_machine_stop_notifier. In
-- this MAME that notifier never fires, so the harness ran, measured
-- everything correctly, and printed NOTHING -- and a gate whose output is
-- empty is a gate that cannot fail. Tennis's own PORTING.md 3.2 is about
-- exactly that shape: a tap that never fired, two consoles that both printed
-- "never reached the snapshot tick", and a verdict that compared the two
-- strings and said ok.
--
-- So the report is driven by the thing being measured. test/run_frames.sh
-- treats a missing FRAMES line as a failure rather than as a pass.
local REPORT_AT = tonumber(os.getenv("FRAME_COUNT") or "900")
local reported = false
_G._fr_tick = emu.add_machine_frame_notifier(function()
    if not reported and n >= REPORT_AT then
        reported = true
        report()
    end
end)
