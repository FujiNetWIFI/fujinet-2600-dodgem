...
; step took into DMFCNT. emu/latency.lua taps those two cells against a VSYNC
...
; The probe draws text -- the endpoint and the round counter -- so it takes the
; cartridge's text primitives out of dmcore.inc. They are behind an IFDEF
; because a bank that draws nothing should not carry them.
DMHASTXT EQU    1
...
; This image contains no game, so its cells are chosen for clarity rather than
; squeezed into a union. They are deliberately NOT at the top of RAM: the stack
; is at $01FF mirrored to $FF and grows down, and a probe that puts its own
; counters under the stack is measuring how deep it nested.
PSTEP   EQU     $90             ; the step id emu/latency.lua taps
PROUND  EQU     $91             ; 2: rounds completed, little-endian
...
DMFCNT  EQU     $93             ; video frames the current transaction has taken
...
;
; DMFCNT counts the frames this transaction took, and is what M1 is for.
PGO:    lda     FNACKS
        clc
        adc     #1
        bne     PGO1
