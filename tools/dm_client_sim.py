#!/usr/bin/env python3
"""dodgem_client_sim.py -- two simulated consoles against a fresh relay.
...
Usage: dodgem_client_sim.py [--port 9639]
"""
...
def relay_argv():
    ...
    server/dodgem_relay_server.py stays the canonical reference
    ...
    if impl == "py":
        return [sys.executable, os.path.join(HERE, "server/dodgem_relay_server.py")]
    if impl == "c":
        binary = os.path.join(HERE, "server/c/dodgem-relay")
...
def main():
    ...
    try:
        ...
        check(pa[4] == pb[4] == 3, "both get the variation the server was told")
