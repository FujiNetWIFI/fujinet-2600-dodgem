#!/usr/bin/env python3
"""dodgem_relay_server.py -- the relay for networked Atari 2600 Dodge 'Em.

Port 9603 -- Combat's room is 9600, Dragster's 9601, Dodge 'Em's 9602. A fork of the Intellivision family's
...
split frame in -- the 128 bytes of console RAM are the GAME'S, every one of
them (Dodge 'Em uses all 128, which is why sixteen live in the cartridge), and
the reply
...
Usage: dodgem_relay_server.py [--port 9603] [--delay 2] [--lobby-url ...]
"""
...
DEFAULT_DELAY = 2           # ticks. A tick is DMK = 4 frames here, so the
                            # delay is 8 frames, about 133 ms, symmetric. The
                            # ROM validates it against its own DMD and refuses
                            # a START that disagrees, so the two cannot drift.
...
class Server:
    ...
    def on_hello(self, c, payload):
        ...
        #
        # DODGE 'EM'S REASON IS COMBAT'S, NOT TENNIS'S, and the distinction is
        # worth keeping because the two refusals look identical and are not.
        # Dodge 'Em refuses because nobody has authored a SECAM palette and looked
        # at it on a real set -- a gap that could be closed. This one cannot be
        # closed: Dodge 'Em draws BOTH cars from one colour table at $FF2C,
        # EOR'd against $A6, and they are told apart by LUMINANCE. A SECAM TIA
        # ignores the luminance bits entirely, so both players would be driving
        # identical cars round the same track and neither could tell which was
        # theirs. That is not a cosmetic loss in a game about not being caught.
        #
        # build.sh refuses to build a SECAM image for the same reason, and the
        # two refusals have to agree or a console would be turned away at the
        # door for declaring something it could not have been built to declare.
        c.tv = payload[1]
        if c.tv == TV_SECAM:
            log("%s: refusing a SECAM console -- Dodge 'Em tells its cars "
                "apart by luminance, which a SECAM TIA ignores", c.addr[0])
            self.drop(c)
            return
...
def main():
    ...
    ap.add_argument("--port", type=int, default=9603)
    ap.add_argument("--delay", type=int, default=DEFAULT_DELAY,
                    help="input delay in ticks (a tick is 4 frames here, and "
                         "a transaction was measured at 3)")
    ap.add_argument("--variation", type=int, default=3,
                    help="the variation a match starts on. ONLY 3 is offered: "
                         "$94's mode bits walk 00 -> 80 -> C0 and only the "
                         "last gives the second player a car to drive. SELECT "
                         "walks $96 & $F8 in the same breath, so the ROM's "
                         "whitelist pins both -- see PORTING.md 6.1.")
    ap.add_argument("--lobby-url", default="",
                    help="e.g. https://lobby.fujinet.online/server; off by default")
    ap.add_argument("--lobby-appkey", type=int, default=27)
    ap.add_argument("--lobby-serverurl", default="TCP://fujinet.online:9603/")
    ap.add_argument("--lobby-client-url",
                    default="TNFS://apps.irata.online/Atari2600/Games/DodgeEm.bin")
    ap.add_argument("--game-name", default="Dodge Em")
    ap.add_argument("--server-name", default="Dodge Em Netplay")
    ap.add_argument("--region", default="us")
    args = ap.parse_args()
    # THREE variations, and only the third is a match.
    #
    # $F162 walks $94's mode bits 00 -> 80 -> C0 -> 00 as SELECT is pressed:
    # one player against the computer, two players alternating with a computer
    # chase car, and two players where whoever is not dodging DRIVES that chase
    # car. Only the last has anything for a second console to do.
    #
    # SELECT walks a SECOND quantity in the same breath -- $96 & $F8 through
    # $58 -> $88 -> $B8 -- so the variation is a PAIR, and the ROM's whitelist
    # pins both. A relay that agreed about $94 alone would pair two consoles
    # playing different levels. See PORTING.md 6.1.
    if args.variation != 3:
        sys.exit("dodgem_relay: variation must be 3 -- the only one where both "
                 "players are active. In 1 and 2 the chase car is driven by "
                 "the computer and the second console has nothing to do.")
    Server(args).run()
